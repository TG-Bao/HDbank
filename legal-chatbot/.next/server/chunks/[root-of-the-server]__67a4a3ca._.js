module.exports = [
"[project]/legal-chatbot/.next-internal/server/app/api/chat-enhanced/route/actions.js [app-rsc] (server actions loader, ecmascript)", ((__turbopack_context__, module, exports) => {
...
"[project]/legal-chatbot/src/app/api/chat-enhanced/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function POST(request) {
    try {
        const cookieStore = (0, __TURBOPACK__imported__module__$5b$project$5d2f$legal$2d$chatbot$2f$node_modules$2f$next$2f$headers$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["cookies"])();
        ...
        const { data: localResults, error: localError } = await supabase.from('laws').select('*').or(`content.ilike.%${query}%,title.ilike.%${query}%`).limit(5);
        let sources = [];
        let matched_ids = [];
        let context = "";
...
async function searchThuvienphapluat(query, limit) {
    try {
        ...
        return mockResults.filter((result)=>result.title.toLowerCase().includes(query.toLowerCase()) || result.content.toLowerCase().includes(query.toLowerCase()) || result.article_reference.toLowerCase().includes(query.toLowerCase())).slice(0, limit);
    } catch (error) {
...
async function searchVanbanchinhphu(query, limit) {
    try {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__67a4a3ca._.js.map